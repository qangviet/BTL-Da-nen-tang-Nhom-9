...
const ClassDocsScreenGVien = ({ route }) => {
	...
	const fetchDocs = async () => {
		try {
			...
			dispatch(stopLoading());
			console.error(
				`Error fetching documents - Api get_material_list - Data: token: ${params.token}, class_id: ${params.class.id}`,
				error
			);
		}
	};
		...
		<View style={{ flex: 1 }}>
			...
			<Modal animationType="fade" transparent={true} visible={modalVisible} onRequestClose={handleCloseModal}>
				<View style={{ flex: 1, backgroundColor: "rgba(0, 0, 0, 0.5)", justifyContent: "flex-end" }}>
					...
					>
						<Text className="text-lg mb-2 self-center">{selectedDoc ? selectedDoc.name : ""}</Text>
						<CustomButton
							title="Mở"
							onPress={() => {
						...
						/>
						<CustomButton title="Xóa" onPress={openModalConfirmDelete} iconName="trash-outline" />
					</View>
					{/* </View> */}
				</View>
				{showConfirmDelete && (
					<Modal transparent={true} animationType="slide" onBackdropPress={closeModalConfirmDelete}>
