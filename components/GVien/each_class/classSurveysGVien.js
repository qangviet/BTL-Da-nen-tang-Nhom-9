...
const ClassSurveys = ({ navigation }) => {
	...
	const sortedDates = Object.keys(groupedAssignments).sort(
		(a, b) => new parseDate(b) - new parseDate(a)
	);
					...
					>
						<Text
							className={`text-center ${
								mode === index ? "text-white" : "text-black"
							}`}
						>
							{label}
						</Text>
						...
						<View className="mb-4">
							...
							{groupedAssignments[item].map((assignmentName, index) => (
								<TouchableOpacity
									...
									key={index}
									className="bg-white p-4 mb-2 ml-2 mr-2 rounded-lg shadow justify-between 
														border border-gray-200 flex-row"
								>
									<View>
