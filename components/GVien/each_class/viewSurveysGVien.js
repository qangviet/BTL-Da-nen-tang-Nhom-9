						...
						{RESPONSES.map((item, index) => (
							...
							>
								...
								<TouchableOpacity
									...
									onPress={() => openModalViewSubmitted(index)}
								>
									<Text className="text-blue-600 underline text-base font-medium">Xem</Text>
								</TouchableOpacity>
							</View>
						))}
					</View>
						...
						<View>
							<View className="mx-4 mt-3">
								<Text className="text-xl mt-1 pl-2 font-semibold">
									{RESPONSES[viewSubmitted].student_name} - {RESPONSES[viewSubmitted].student_id}
								</Text>
								<Text className="text-base mt-4 px-2">{RESPONSES[viewSubmitted].text_response}</Text>
							</View>
							<View className="flex flex-row px-4 mt-3">
								<TouchableOpacity
									className="px-2 py-2"
									onPress={() => openMaterial(RESPONSES[viewSubmitted].file_url)}
								>
							...
							<View className="flex flex-row justify-center pt-8 gap-x-1">
								...
								<TextInput
									className="text-sm px-3 border border-gray-600"
									defaultValue={
										RESPONSES[viewSubmitted].grade != null
											? RESPONSES[viewSubmitted].grade.toString()
											: ""
									}
									onChangeText={(text) => setTempGrade(text)}
								/>
								<Text className="text-lg">/ 10</Text>
							</View>
							<TouchableOpacity
								className="px-4 py-2 rounded-lg bg-red-600 w-[30%] mx-auto mt-10 mb-10"
								onPress={() => gradingResponse(RESPONSES[viewSubmitted])}
							>
								<Text className="text-white text-lg font-bold self-center">Save</Text>
							</TouchableOpacity>
						</View>
					</View>
				</Modal>
